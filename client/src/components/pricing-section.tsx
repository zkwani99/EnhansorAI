        ...
        <div className="mt-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {creditPacks.map((pack: CreditPack, index: number) => {
              const getIcon = () => {
                if (index === 0) return <Zap className="w-6 h-6 text-purple-600" />;
                if (index === 1) return <Sparkles className="w-6 h-6 text-purple-600" />;
                if (index === 2) return <Crown className="w-6 h-6 text-purple-600" />;
                return <Building2 className="w-6 h-6 text-purple-600" />;
              };

              const isPopular = pack.isPopular || index === 1; // Default second pack as popular if not set
              const pricePerCredit = (pack.price / pack.credits / 100).toFixed(3);

              return (
                <Card 
                  key={pack.id} 
                  className={`bg-white dark:bg-black rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer transform hover:scale-105 ${
                    isPopular 
                      ? 'border-2 border-purple-300 ring-2 ring-purple-100 relative' 
                      : 'border border-gray-200 hover:border-purple-300'
                  }`}
                >
                  {isPopular && (
                  ...
                  <CardContent className="p-6 text-center h-full flex flex-col">
                    <div className="mb-4">
                      ...
                      <div className="mb-4">
                        <span className="text-3xl font-bold text-purple-600">{pack.displayPrice}</span>
                      ...
                      <p className="text-sm text-gray-800 dark:text-gray-200">${pricePerCredit} per credit</p>
                    </div>
                    <div className="mt-auto">
                      <Button 
                        ...
                        data-testid={`button-buy-${pack.name.toLowerCase().replace(' ', '-')}`}
                      >
                        Buy {pack.name}
                      </Button>
                    </div>
